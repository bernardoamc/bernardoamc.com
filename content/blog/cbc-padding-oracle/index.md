...
In this post we will investigate the byte-at-a-time ECB decryption exercise from [Cryptopals](https://cryptopals.com/sets/3/challenges/17). I highly recommend attempting the previous exercises yourself as they do a great job ramping up your knowledge on the subject. We also have a [previous post](/cbc-bitflipping-attack) explaining how `CBC` works.
...
If you are used to the `XOR` operation you will realize right away that the only way for `AES_DEC(ciphertext_block) XOR Zeroing IV` to output zero is if they are **equal**! This means that we have just computed the value of `AES_DEC(ciphertext_block)`, which was the only thing we didn't have access to.

To output our plaintext message all we need to do now is: `Zeroing IV XOR Our original IV`.
...
We want to make the operation `AES_DEC(ciphertext_block) XOR Zeroing IV` return an array with the final byte set to `1` so our `padding_oracle` method returns true. In order to do that we need to find a value that makes `33 XOR <something> = 1`:
...
Now we need to make the operation `AES_DEC(ciphertext_block) XOR Zeroing IV` return an array with the final and penultimate bytes set to `2` so our `padding_oracle` method returns true. In order to do that we need to:
...
The next iteration will make the last three bytes equal to `3` and we are going to find yet another byte, and so on and so forth until we decrypt an entire block, so eventually our `Zeroing IV` will become the same as our `AES_DEC(ciphertext_block)`: <br />
`[99, 111, 110, 103, 114, 97, 116, 117, 108, 97, 116, 105, 111, 110, 115, 33]`
...
# Our boilerplate is done, let's now implement it!
