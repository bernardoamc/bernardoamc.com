...
description: "Let's investigate how encrypting and decrypting in CTR mode works."
---

So far we have investigated [ECB](/ecb-decryption-simple) and [CBC](/cbc-bitflipping-attack), now it's time to explore the `Counter (CTR) mode`. Let's start with a definition from Wikipedia:
...
The first step is to generate the `Nonce + Counter`. In this example we will only concated both arrays and end up with: <br/>
...
Followed by `XOR`ing this value with our first block from the plaintext:
...
with the amount of bytes that our plaintext block has remaining, which is a single one for this example! So:
...
Now we just put all of our results together and end up with our `ciphertext`:<br/>
...
def aes_ctr_encrypt(plaintext, key, nonce)
  ...
  blocks.each_with_index.flat_map do |block, counter|
    # Let's make our counter occupy 8 bytes
...
The cool thing about this algorithm is that the encryption is **exactly the same as the decryption**, we only need to provide the ciphertext to our algorithm instead of the plaintext.
...
# Notice that we are passing the CIPHERTEXT now
decryption =  aes_ctr_encrypt(CIPHERTEXT, KEY, NONCE)
puts decryption == PLAINTEXT # true
```

And we have understood yet another block cipher mode!

In our future posts we will see attacks involving CTR mode and how they can be prevented. Reach out to me via email or Twitter if you have suggestions, questions or just want to chat about the topic. :)
